...
db_add_routing_rule() {
    ...
    if [[ "$rule_type" == "custom" ]]; then
        jq --arg id "$rule_id" --arg type "$rule_type" --arg out "$outbound" --arg domains "$rule_domains" --arg ipv6 "$ipv6_only" \
            '.routing_rules = ((.routing_rules // []) + [{id: $id, type: $type, outbound: $out, domains: $domains, ipv6_only: ($ipv6 == "true")}])' \
...
_add_routing_rule() {
    ...
    # Netflix 特殊处理：询问是否仅使用 IPv6
    local ipv6_only="false"
    if [[ "$rule_type" == "netflix" ]]; then
        echo ""
        echo -e "  ${Y}Netflix IPv6 设置:${NC}"
        echo -e "  ${D}使用 IPv6 可避免同户检测和封号风险${NC}"
        read -rp "  是否仅使用 IPv6 出站? [y/N]: " use_ipv6
        [[ "$use_ipv6" =~ ^[Yy]$ ]] && ipv6_only="true"
    fi
