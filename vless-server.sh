...
_select_outbound() {
    ...
    for info in "${display_names[@]}"; do
        if [[ "$info" == "WARP" ]]; then
            ...
            local result=$(check_node_latency "$server" "$port" 2>/dev/null)
...
show_routing_status() {
    ...
    if [[ -n "$rules" && "$rules" != "[]" ]]; then
        local rule_count=0
        # 一次性提取 type 和 outbound，用 | 分隔
        while IFS='|' read -r rule_type outbound; do
            ...
            ((rule_count++))
        done < <(echo "$rules" | jq -r '.[] | "\(.type)|\(.outbound)"')
...
test_routing() {
    ...
    while IFS= read -r rule; do
        ...
        if [[ "$rule_type" == "all" ]]; then
            ...
            local test_domain=""
            [[ -n "$domains" ]] && test_domain=$(echo "$domains" | cut -d',' -f1)
...
add_quick_proxy() {
    ...
    # 检查是否已存在同名节点
    if db_get_chain_node "$node_name" | jq -e . >/dev/null 2>&1; then
...
# 检测节点延迟和解析 IP
# 用法: check_node_latency "server" "port"
# 返回: "延迟ms|IP" 或 "超时|-"
check_node_latency() {
...
manage_chain_proxy() {
    while true; do
        ...
        case "$choice" in
            1)
                echo ""
                echo -e "  ${Y}粘贴代理链接 (支持 ss/vmess/vless/trojan/hy2):${NC}"
                read -rp "  链接: " link
                [[ -z "$link" ]] && continue
            ...
            2)
                ...
                if [[ -n "$parsed_nodes" ]]; then
                    ...
                    _ok "已添加 $added 个节点"
                    [[ $failed -gt 0 ]] && _warn "跳过 $failed 个无效节点"
                fi
                _pause
                ;;
            3)
                # 测试所有节点延迟
                _header
                echo -e "  ${W}测试节点延迟 ${D}(TCP连接延迟，仅供参考)${NC}"
                _line
...
show_status() {
    ...
    if [[ "$rules_count" -gt 0 ]]; then
        local warp_st=$(warp_status)
        local chain_active=$(db_get_chain_active)
        ...
        if [[ "$first_outbound" == chain:* ]]; then
            # 链式代理出口
            if [[ -n "$chain_active" ]]; then
                echo -e "  分流: ${G}${rules_count}条规则→链式代理${NC}"
            else
                echo -e "  分流: ${Y}${rules_count}条规则→链式代理 (未选择节点)${NC}"
            fi
