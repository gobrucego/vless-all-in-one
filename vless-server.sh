#!/bin/bash
#═══════════════════════════════════════════════════════════════════════════════
#  多协议代理一键部署脚本 v3.0.1 [服务端]
#  支持协议: VLESS+Reality / VLESS+Reality+XHTTP / VLESS+WS / VMess+WS / 
#           VLESS-XTLS-Vision / SOCKS5 / SS2022 / HY2 / Trojan / 
#           Snell v4 / Snell v5 / AnyTLS / TUIC / 
#           Snell v4+ShadowTLS / Snell v5+ShadowTLS / SS2022+ShadowTLS (共16种)
...
gen_snell_shadowtls_server_config() {
    ...
    # Snell 后端端口 (内部监听)
    local snell_backend_port=$((port + 10000))
...
gen_ss2022_shadowtls_server_config() {
    local password="$1" port="$2" method="${3:-2022-blake3-aes-256-gcm}" sni="${4:-www.microsoft.com}" stls_password="$5"
    mkdir -p "$CFG"
...
select_protocol() {
    ...
    _item "7" "Shadowsocks 2022 ${D}(新版加密)${NC}"
    _item "8" "Hysteria2 ${D}(UDP加速, 高速)${NC}"
    _item "9" "Trojan ${D}(TLS主协议, 支持回落)${NC}"
    _item "10" "Snell v4 ${D}(Surge专用)${NC}"
    _item "11" "Snell v5 ${D}(Surge 5.0新版)${NC}"
    _item "12" "AnyTLS ${D}(多协议TLS代理)${NC}"
    _item "13" "Snell v4 + ShadowTLS ${D}(Surge+TLS伪装)${NC}"
    _item "14" "Snell v5 + ShadowTLS ${D}(Surge5+TLS伪装)${NC}"
    _item "15" "SS2022 + ShadowTLS ${D}(SS+TLS伪装)${NC}"
    _item "16" "TUIC v5 ${D}(QUIC协议)${NC}"
    echo ""
    echo -e "  ${D}提示: 先装主协议(5/9)占用443，再装WS(3/4)可共用端口${NC}"
    echo ""
    
    while true; do
        read -rp "  选择协议 [1-16]: " choice
        case $choice in
            ...
            7) SELECTED_PROTOCOL="ss2022"; break ;;
            ...
            13) SELECTED_PROTOCOL="snell-shadowtls"; break ;;
            14) SELECTED_PROTOCOL="snell-v5-shadowtls"; break ;;
            15) SELECTED_PROTOCOL="ss2022-shadowtls"; break ;;
            16) SELECTED_PROTOCOL="tuic"; break ;;
            *) _err "无效选择" ;;
        esac
    done
}
...
do_install_server() {
    ...
    case "$protocol" in
        ...
        anytls)
            ...
            ;;
        snell-shadowtls)
            local psk=$(gen_password) stls_password=$(gen_password)
            local default_sni=$(gen_sni)
            
            echo ""
            read -rp "  SNI 伪装域名 [$default_sni]: " final_sni
            ...
            _info "生成配置..."
            gen_snell_shadowtls_server_config "$psk" "$port" "$final_sni" "$stls_password" "4"
            ;;
        snell-v5-shadowtls)
            local psk=$(gen_password) stls_password=$(gen_password)
            local default_sni=$(gen_sni)
            ...
            _info "生成配置..."
            gen_snell_shadowtls_server_config "$psk" "$port" "$final_sni" "$stls_password" "5"
            ;;
        ss2022-shadowtls)
            # SS2022 加密方式选择
            echo ""
            ...
            _info "生成配置..."
            gen_ss2022_shadowtls_server_config "$password" "$port" "$method" "$final_sni" "$stls_password"
            ;;
        snell-v5)
