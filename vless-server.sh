...
EOF
    ...
    _write_systemd() { # name desc exec pre before env
        local name="$1" desc="$2" exec="$3" pre="$4" before="$5" env="$6"
...
EOF
    ...
    if [[ "$DISTRO" == "alpine" ]]; then
        ...
        _write_systemd "$service_name" "Proxy Server ($protocol)" "$exec_cmd" "$pre" "" "$env"
...
show_status() {
    ...
    # 一次 jq 调用，输出格式: XRAY:proto1,proto2 SINGBOX:proto3 PORTS:proto1=443,proto2=8080 ROUTING:type
    ...
    # 解析结果
    local xray_keys="" singbox_keys="" routing_type="" ports_map=""
    local part
    for part in $db_parsed; do
        case "$part" in
            XRAY:*) xray_keys="${part#XRAY:}" ;;
            SINGBOX:*) singbox_keys="${part#SINGBOX:}" ;;
            ROUTING:*) routing_type="${part#ROUTING:}" ;;
    ...
    # 显示分流状态
    if [[ -n "$routing_type" && "$routing_type" != "disabled" && "$routing_type" != "null" ]]; then
        local warp_st=$(warp_status)
        local routing_desc=""
        case "$routing_type" in
