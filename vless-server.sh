#!/bin/bash
#═══════════════════════════════════════════════════════════════════════════════
#  多协议代理一键部署脚本 v3.0.4 [服务端]
#  
#  架构升级:
#    • Xray 核心: 处理 TCP/TLS 协议 (VLESS/VMess/Trojan/SOCKS/SS2022)
#    • Sing-box 核心: 处理 UDP/QUIC 协议 (Hysteria2/TUIC) - 低内存高效率
#    • JSON 状态数据库: 替代 .info 文件，原子操作，jq 安全构建配置
#  
...
#═══════════════════════════════════════════════════════════════════════════════
#  全局状态数据库 (JSON) - 替代 .info 文件，提高效率和原子性
#═══════════════════════════════════════════════════════════════════════════════
...
init_db() {
    ...
    tmp=$(mktemp) || return 1
    jq -n --arg v "4.0.0" --arg t "$now" \
      '{version:$v,xray:{},singbox:{},meta:{created:$t,updated:$t}}' >"$tmp" &&
      mv "$tmp" "$DB_FILE"
}

# 迁移旧配置到数据库（占位函数，新安装无需迁移）
migrate_info_to_db() {
    # 数据库已存在且有数据时跳过迁移
    [[ -f "$DB_FILE" ]] && return 0
    return 0
...
_db_touch() {
    ...
    now=$(date -Iseconds 2>/dev/null || date '+%Y-%m-%dT%H:%M:%S')
    tmp=$(mktemp) || return 1
    jq --arg t "$now" '.meta.updated=$t' "$DB_FILE" >"$tmp" && mv "$tmp" "$DB_FILE"
}
...
db_add() { # db_add core proto json
    _db_apply --arg p "$2" --argjson c "$3" ".${1}[\$p]=\$c"
}
...
# 构建并保存协议配置到数据库
# 用法: save_protocol_config "协议名" "核心类型" "key1=val1" "key2=val2" ...
# 示例: save_protocol_config "vless" "xray" "uuid=$uuid" "port:n=$port" "sni=$sni"
# 注意: key:n 表示数字类型，否则为字符串
save_protocol_config() {
    ...
    for kv in "$@"; do
        ...
        # 检查是否为数字类型 (key:n=value)
        if [[ "$key" == *":n" ]]; then
            key="${key%:n}"
            is_num=true
        fi
        
        [[ "$first" == "true" ]] && first=false || jq_obj+=","
    ...
    # 添加 IP
    jq_args+=(--arg ipv4 "$ipv4" --arg ipv6 "$ipv6")
    jq_obj+=",\"ipv4\":\$ipv4,\"ipv6\":\$ipv6}"
    
    # 生成 JSON 并保存
    local config_json=$(jq -n "${jq_args[@]}" "$jq_obj")
    register_protocol "$protocol" "$config_json"
}

# 简化版：直接用关联数组构建 JSON (更灵活)
...
register_protocol() {
    ...
    if [[ -n "$config_json" ]]; then
        local core="xray"
        # 判断协议归属的核心
        if echo "$SINGBOX_PROTOCOLS" | grep -qw "$protocol"; then
            core="singbox"
        elif echo "$STANDALONE_PROTOCOLS" | grep -qw "$protocol"; then
...
# 生成 Xray 多 inbounds 配置 (重构版 - 使用 jq 动态构建)
generate_xray_config() {
    ...
    # 检查是否有分流配置
    local routing_type=$(db_get_routing_type)
    local warp_outbound=""
    local routing_rules=""
    
    if [[ -n "$routing_type" && "$routing_type" != "disabled" ]]; then
        warp_outbound=$(gen_xray_warp_outbound)
        routing_rules=$(gen_xray_routing_rules)
    fi
    
    # 使用 jq 构建基础配置
    if [[ -n "$warp_outbound" && "$warp_outbound" != "" ]]; then
        # 有分流配置且 WARP 已安装：添加 WARP outbound 和路由规则
        jq -n --argjson warp "$warp_outbound" '{
            log: {loglevel: "warning"},
            inbounds: [],
            outbounds: [{protocol: "freedom", tag: "direct"}, $warp],
        ...
        # 无分流配置或 WARP 未安装：简单配置
        if [[ -n "$routing_type" && "$routing_type" != "disabled" && -z "$warp_outbound" ]]; then
            _warn "已配置分流规则但 WARP 未安装，分流不会生效"
        fi
        jq -n '{
            log: {loglevel: "warning"},
            inbounds: [],
...
# 生成 Sing-box 统一配置 (Hy2 + TUIC + SS2022 共用一个进程)
generate_singbox_config() {
    ...
    if [[ -n "$routing_type" && "$routing_type" != "disabled" ]]; then
        warp_outbound=$(gen_singbox_warp_outbound)
        routing_rule=$(gen_singbox_routing_rules)
    fi
    ...
    if [[ -n "$warp_outbound" && "$warp_outbound" != "" ]]; then
        # 有分流配置且 WARP 已安装
        base_config=$(jq -n --argjson warp "$warp_outbound" '{
            log: {level: "warn", timestamp: true},
            inbounds: [],
            outbounds: [{type: "direct", tag: "direct"}, $warp],
            route: {rules: []}
        }')
        
        # 添加路由规则
        if [[ -n "$routing_rule" ]]; then
            base_config=$(echo "$base_config" | jq --argjson rule "$routing_rule" '.route.rules = [$rule]')
        fi
    else
        # 无分流配置或 WARP 未安装
        if [[ -n "$routing_type" && "$routing_type" != "disabled" && -z "$warp_outbound" ]]; then
    ...
    for proto in $singbox_protocols; do
        ...
        case "$proto" in
            ...
            ss2022)
                local password=$(echo "$cfg" | jq -r '.password // empty')
                local method=$(echo "$cfg" | jq -r '.method // "2022-blake3-aes-128-gcm"')
...
gen_tuic_server_config() {
    ...
    # 写入数据库（Sing-box 从数据库读取配置生成 singbox.json）
    register_protocol "tuic" "$(build_config uuid "$uuid" password "$password" port "$port" sni "$sni")"
...
create_service() {
    ...
    case "$kind" in
        ...
        shadowtls)
            ...
            exec_cmd="/usr/local/bin/shadow-tls --v3 server --listen 0.0.0.0:${port} --server 127.0.0.1:${ss_backend_port:-$snell_backend_port} --tls ${sni}:443 --password ${stls_password}"
...
EOF
    ...
    _write_systemd() { # name desc exec pre before
        local name="$1" desc="$2" exec="$3" pre="$4" before="$5"
...
EOF
    ...
    if [[ "$DISTRO" == "alpine" ]]; then
        ...
        _write_systemd "$service_name" "Proxy Server ($protocol)" "$exec_cmd" "$pre" ""
    ...
    if [[ "$DISTRO" == "alpine" ]]; then
        case "$action" in
            ...
            status)
                ...
                local p="${SVC_PROC[$name]:-}"; [[ -n "$p" ]] && pgrep -x "$p" >/dev/null 2>&1
                ;;
        esac
    else
        case "$action" in
...
gen_xray_warp_outbound() {
    ...
    [[ -z "$routing_type" || "$routing_type" == "disabled" ]] && return
...
test_warp_connection() {
    ...
    # 检查是否有分流规则
    local routing_type=$(db_get_routing_type)
    if [[ -z "$routing_type" || "$routing_type" == "disabled" ]]; then
...
configure_warp_official() {
    ...
    if [[ "$is_registered" != "true" ]]; then
        ...
        reg_output=$(warp-cli registration new --accept-tos 2>&1)
        if [[ $? -eq 0 ]]; then
            reg_success=true
        elif echo "$reg_output" | grep -qi "already"; then
            # 已经注册过了
            reg_success=true
        else
            # 尝试旧版命令
            reg_output=$(warp-cli register --accept-tos 2>&1)
            if [[ $? -eq 0 ]]; then
                reg_success=true
            elif echo "$reg_output" | grep -qi "already"; then
                reg_success=true
...
# 数据库：保存分流规则
db_set_routing() {
    local rule_type="$1"  # warp_all, warp_openai, warp_netflix, custom
    local custom_domains="$2"
    
    [[ ! -f "$DB_FILE" ]] && echo '{}' > "$DB_FILE"
    
    local tmp=$(mktemp)
    # 使用 += 合并，保留 warp_mode 等其他字段
    jq --arg type "$rule_type" --arg domains "$custom_domains" \
        '.routing = (.routing // {}) + {type: $type, custom_domains: $domains}' "$DB_FILE" > "$tmp" && mv "$tmp" "$DB_FILE"
}

# 数据库：获取分流规则
db_get_routing() {
    [[ ! -f "$DB_FILE" ]] && return
    jq -r '.routing // empty' "$DB_FILE" 2>/dev/null
}

db_get_routing_type() {
    [[ ! -f "$DB_FILE" ]] && return
    jq -r '.routing.type // empty' "$DB_FILE" 2>/dev/null
}

db_get_routing_domains() {
    [[ ! -f "$DB_FILE" ]] && return
    jq -r '.routing.custom_domains // empty' "$DB_FILE" 2>/dev/null
}
...
db_del_routing() {
    [[ ! -f "$DB_FILE" ]] && return
    local tmp=$(mktemp)
    jq 'del(.routing)' "$DB_FILE" > "$tmp" && mv "$tmp" "$DB_FILE"
}

# 预设分流规则
ROUTING_OPENAI_DOMAINS="openai.com,chatgpt.com,chat.openai.com,ai.com,sora.com"
ROUTING_NETFLIX_DOMAINS="netflix.com,netflix.net,nflximg.net,nflximg.com,nflxvideo.net,nflxso.net,nflxext.com"
ROUTING_DISNEY_DOMAINS="disney.com,disneyplus.com,dssott.com,bamgrid.com,disney-plus.net"
ROUTING_STREAMING_DOMAINS="$ROUTING_NETFLIX_DOMAINS,$ROUTING_DISNEY_DOMAINS,hulu.com,hbo.com,hbomax.com,spotify.com"

# 获取分流域名列表 (公共函数)
_get_routing_domains() {
    local routing_type=$(db_get_routing_type)
    case "$routing_type" in
        warp_openai)   echo "$ROUTING_OPENAI_DOMAINS" ;;
        warp_netflix)  echo "$ROUTING_NETFLIX_DOMAINS" ;;
        warp_streaming) echo "$ROUTING_STREAMING_DOMAINS" ;;
        custom)        db_get_routing_domains ;;
        *)             echo "" ;;
    esac
}
...
gen_xray_routing_rules() {
    ...
    # 所有流量走 WARP
    if [[ "$routing_type" == "warp_all" ]]; then
        echo '[{"type": "field", "network": "tcp,udp", "outboundTag": "warp"}]'
        return
    fi
    
    local domains=$(_get_routing_domains)
    [[ -z "$domains" ]] && return
    
    # 转换为 JSON 数组，添加 domain: 前缀实现子域名匹配
    local domain_array=$(echo "$domains" | tr ',' '\n' | sed 's/^/domain:/' | jq -R . | jq -s .)
    jq -n --argjson domains "$domain_array" '[{"type": "field", "domain": $domains, "outboundTag": "warp"}]'
}
...
gen_singbox_routing_rules() {
    ...
    # 转换为 Sing-box 格式 (domain_suffix 数组)
    local domain_array=$(echo "$domains" | tr ',' '\n' | jq -R . | jq -s .)
    jq -n --argjson domains "$domain_array" '{"domain_suffix": $domains, "outbound": "warp"}'
}
...
show_routing_status() {
    ...
    case "$warp_st" in
        connected)
            ...
            echo -e "  代理: ${C}127.0.0.1:${WARP_OFFICIAL_PORT}${NC}"
            ;;
        registered)
            echo -e "  WARP: ${Y}● 已注册${NC} (官方客户端/未连接)"
            ;;
        configured)
    ...
    case "$routing_type" in
        warp_openai)
            echo -e "  分流: ${G}OpenAI/ChatGPT${NC} → WARP"
            ;;
        warp_netflix)
            echo -e "  分流: ${G}Netflix${NC} → WARP"
            ;;
        warp_streaming)
            echo -e "  分流: ${G}流媒体 (Netflix/Disney+/HBO)${NC} → WARP"
            ;;
        warp_all)
            echo -e "  分流: ${Y}所有流量${NC} → WARP"
            ;;
        custom)
            local domains=$(db_get_routing_domains)
            echo -e "  分流: ${C}自定义规则${NC}"
            [[ -n "$domains" ]] && echo -e "  域名: ${D}${domains:0:50}...${NC}"
            ;;
        *)
            echo -e "  分流: ${D}未配置${NC}"
            ;;
...
test_routing() {
    local routing_type=$(db_get_routing_type)
    [[ -z "$routing_type" || "$routing_type" == "disabled" ]] && return 0
    
    local warp_st=$(warp_status)
    if [[ "$warp_st" != "configured" && "$warp_st" != "connected" ]]; then
        _warn "WARP 未配置或未连接，分流无法生效"
        return 1
    fi
    ...
    # 显示当前模式
    if [[ "$warp_mode" == "official" ]]; then
        echo -e "  WARP 模式: ${G}官方客户端 (TCP/SOCKS5)${NC}"
        ...
        if [[ -n "$warp_ip" ]]; then
            ...
            echo -e "  WARP 出口 IP: ${Y}获取失败 (可能是网络问题)${NC}"
        fi
    else
        echo -e "  WARP 模式: ${C}WGCF (UDP/WireGuard)${NC}"
        echo -e "  ${D}WARP 出口 IP 需通过代理访问目标网站验证${NC}"
    fi
    
    # 获取本机直连 IP
    local direct_ip=$(curl -s --connect-timeout 5 https://api.ipify.org 2>/dev/null)
    if [[ -z "$direct_ip" ]]; then
        direct_ip=$(curl -s --connect-timeout 5 https://ifconfig.me 2>/dev/null)
    fi
    ...
    case "$routing_type" in
        ...
        warp_streaming)
            test_domain="www.netflix.com"
            test_desc="流媒体(Netflix)"
            ;;
        warp_all)
            echo -e "  ${G}✓${NC} 所有流量已配置走 WARP"
            echo -e "  ${D}通过代理访问任意网站将使用 WARP 出口${NC}"
            return 0
            ;;
        custom)
            local domains=$(db_get_routing_domains)
            test_domain=$(echo "$domains" | cut -d',' -f1)
            test_desc="自定义域名"
            ;;
    esac
    
    if [[ -n "$test_domain" ]]; then
        echo -e "  测试 ${test_desc} 分流: ${C}${test_domain}${NC}"
        
        # DNS 解析测试
        local resolved_ip=$(dig +short "$test_domain" 2>/dev/null | head -1)
        if [[ -n "$resolved_ip" ]]; then
            echo -e "  ${D}域名解析: ${test_domain} → ${resolved_ip}${NC}"
        fi
        
        echo ""
        echo -e "  ${G}✓${NC} 分流规则已配置"
        echo -e "  ${D}访问 ${test_domain} 将通过 WARP 出口${NC}"
        echo ""
        echo -e "  ${Y}验证方法:${NC} 手机访问 https://ip.sb 查看出口 IP"
    fi
...
configure_routing_rules() {
    ...
    if [[ "$warp_st" != "configured" && "$warp_st" != "connected" ]]; then
        echo ""
        _warn "WARP 未配置，需要先配置 WARP"
        echo ""
        echo -e "  ${Y}选择 WARP 模式:${NC}"
        _item "1" "WGCF 模式 (UDP/WireGuard) - 性能好"
        _item "2" "官方客户端 (TCP/SOCKS5) - 绕过 UDP 封锁"
        _item "0" "返回"
        _line
        read -rp "  请选择: " mode_choice
        ...
        case "$mode_choice" in
            ...
            2)
                if [[ "$DISTRO" == "alpine" ]]; then
                    _err "Alpine 不支持官方客户端，请使用 WGCF"
                    _pause
                    return
                fi
                if install_warp_official && configure_warp_official; then
                    _regenerate_proxy_configs
                fi
                ;;
            0|*)
    ...
    echo -e "  ${Y}选择分流模式:${NC}"
    echo ""
    _item "1" "OpenAI/ChatGPT 解锁"
    _item "2" "Netflix 解锁"
    _item "3" "流媒体解锁 (Netflix/Disney+/HBO)"
    _item "4" "所有流量走 WARP"
    _item "5" "自定义域名规则"
    _item "6" "关闭分流"
    _item "0" "返回"
    ...
    read -rp "  请选择: " choice
    choice=$(echo "$choice" | tr -d ' \t')  # 去掉空格和 tab
    
    local need_restart=false
    
    case "$choice" in
        1)
            db_set_routing "warp_openai" ""
            _ok "已设置 OpenAI 分流规则"
            need_restart=true
            ;;
        2)
            db_set_routing "warp_netflix" ""
            _ok "已设置 Netflix 分流规则"
            need_restart=true
            ;;
        3)
            db_set_routing "warp_streaming" ""
            _ok "已设置流媒体分流规则"
            need_restart=true
            ;;
        4)
            db_set_routing "warp_all" ""
            _ok "已设置所有流量走 WARP"
            need_restart=true
            ;;
        5)
            ...
            echo -e "  ${D}例如: openai.com,netflix.com,google.com${NC}"
            read -rp "  域名: " custom_domains
            custom_domains=$(echo "$custom_domains" | tr -d ' \t')  # 去掉空格
            if [[ -n "$custom_domains" && "$custom_domains" =~ \. ]]; then
                db_set_routing "custom" "$custom_domains"
                _ok "已设置自定义分流规则"
                need_restart=true
            else
                _warn "域名格式无效，请输入如 openai.com,netflix.com"
            fi
            ;;
        6)
            db_set_routing "disabled" ""
            _ok "已关闭分流"
            need_restart=true
            ;;
        0|"")
...
uninstall_specific_protocol() {
    ...
    elif echo "$SINGBOX_PROTOCOLS" | grep -qw "$selected_protocol"; then
...
do_install_server() {
    ...
    if is_protocol_installed "$protocol"; then
        ...
        if [[ "$reinstall" =~ ^[yY]$ ]]; then
            ...
            elif echo "$SINGBOX_PROTOCOLS" | grep -qw "$protocol"; then
                # Sing-box 协议 (hy2/tuic)：清理特定资源
                
                # Hysteria2: 先清理 iptables 端口跳跃规则
                [[ "$protocol" == "hy2" ]] && cleanup_hy2_nat_rules
...
show_status() {
    ...
    if [[ -n "$installed" ]]; then
        ...
        [[ -n "$singbox_protocols" ]] && singbox_count=$(echo "$singbox_protocols" | wc -l)
        ...
        if [[ "$singbox_running" == "true" ]]; then
            running_protocols=$((running_protocols + singbox_count))
        fi
        ...
        if [[ $protocol_count -eq 1 ]]; then
            local port=""
            # 尝试从 xray 或 singbox 读取 (standalone 也存在 singbox 下)
            if db_exists "xray" "$installed"; then
                port=$(db_get_field "xray" "$installed" "port")
            elif db_exists "singbox" "$installed"; then
                port=$(db_get_field "singbox" "$installed" "port")
            fi
            ...
            for proto in $installed; do
                ...
                # 尝试从 xray 或 singbox 读取 (standalone 也存在 singbox 下)
                if db_exists "xray" "$proto"; then
                    proto_port=$(db_get_field "xray" "$proto" "port")
                elif db_exists "singbox" "$proto"; then
                    proto_port=$(db_get_field "singbox" "$proto" "port")
                fi
        ...
        if [[ -n "$routing_type" && "$routing_type" != "disabled" ]]; then
            ...
            local routing_desc=""
            case "$routing_type" in
                warp_openai) routing_desc="OpenAI" ;;
                warp_netflix) routing_desc="Netflix" ;;
                warp_streaming) routing_desc="流媒体" ;;
                warp_all) routing_desc="全部流量" ;;
                custom) routing_desc="自定义" ;;
            esac
            if [[ "$warp_st" == "configured" || "$warp_st" == "connected" ]]; then
                echo -e "  分流: ${G}${routing_desc}→WARP${NC}"
            else
                echo -e "  分流: ${Y}${routing_desc}→WARP (未运行)${NC}"
            fi
        fi
    else
        echo -e "  状态: ${D}○ 未安装${NC}"
...
main_menu() {
    ...
    while true; do
        _header
        echo -e "  ${W}服务端管理${NC}"
        echo -e "  ${D}系统: $DISTRO | 架构: Xray+Sing-box 双核${NC}"
        echo ""
        show_status
        echo ""
        _line
