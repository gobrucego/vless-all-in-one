...
一个简单易用的多协议代理部署脚本，支持 **16 种主流协议**，服务端/客户端一键安装，适用于 Alpine、Debian、Ubuntu、CentOS 等 Linux 发行版。
...
### 📦 脚本架构重构
- 原一键脚本拆分为独立的 **服务端** (`vless-server.sh`) 和 **客户端** (`vless-client.sh`)
...
### 🌐 外部节点管理 (多机聚合订阅)
- **聚合多台服务器节点**到统一订阅链接，一个订阅管理所有小鸡
- 支持添加分享链接：`vless://`、`vmess://`、`trojan://`、`ss://`、`hysteria2://`、`anytls://`
- 支持添加订阅链接：V2Ray/Base64 订阅、Clash YAML 订阅自动解析
- 外部节点自动添加 IP 后缀避免名称冲突

### 📛 订阅节点命名优化
- 节点名称格式：`地区-协议名-IP后缀`（如 `DE-VLESS+Reality-126`、`US-Hysteria2-183`）
- 自动获取服务器地区代码，Clash/Surge/V2Ray 订阅统一命名

### 🌍 IPv6 完整支持
- 分享链接解析支持 IPv6 格式 `[2001:db8::1]:port`
- Clash 配置兼容 IPv6 地址，不再报 YAML 解析错误

### ⚙️ 用户配置区
- 脚本头部新增配置区，JOIN 码显示开关 `SHOW_JOIN_CODE="on/off"`
...
| 10 | **Snell v4** | Surge 专用协议 | iOS/Mac 用户 |
| 11 | **Snell v5** | Surge 5.0 新版协议 | 最新 Surge |
| 12 | **AnyTLS** | 多协议 TLS 代理 | 抗审查能力强 |
| 13 | **TUIC v5** | QUIC 协议，低延迟 | 新兴协议 |
| 14 | **Snell v4 + ShadowTLS** | Snell 套壳，流量伪装 | 🆕 抗检测 |
| 15 | **Snell v5 + ShadowTLS** | Snell v5 套壳 | 🆕 最新 Surge |
| 16 | **SS2022 + ShadowTLS** | SS2022 套壳，抗检测 | 🆕 高隐蔽性 |
