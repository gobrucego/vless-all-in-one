...
### ✨ 新功能
- 分流规则新增「直连」出口选项，可指定流量走本机 IP 出口
- 直连规则优先级最高，确保指定流量不经过落地节点
- 分流规则优先级说明：直连规则 > 自定义规则 > 预设规则 > 全局规则
...
```
直连规则 > 自定义规则 > 预设规则 (OpenAI/Netflix等) > 全局规则 (all)
```

- **直连规则**：指定走本机 IP 的规则，优先级最高
- **自定义规则**：用户添加的自定义域名/IP 规则
- **预设规则**：内置的 OpenAI、Netflix、TikTok 等规则
- **全局规则**：匹配所有流量的兜底规则
...
1) 直连 (本机 IP 出口)
2) WARP
3) [10ms] 🇭🇰Hong Kong 03 (vless) 123.123.123.123
4) [10ms] 🇭🇰Hong Kong 04 (vless) 123.123.123.123
5) [44ms] 🇸🇬Singapore 04 (vless) 123.123.123.123
...
```

### 广告屏蔽

支持 `geosite:category-ads-all` 规则，自动拦截数万条广告/追踪域名：

```bash
vless
# 分流管理 → 配置分流规则 → 添加分流规则 → b) 广告屏蔽 (geosite)
```

### 预设分流规则

脚本内置以下 geosite 规则，覆盖更全面且自动更新：
...
支持添加自定义域名或 IP 地址的分流规则：

```bash
vless
# 分流管理 → 配置分流规则 → 添加分流规则 → c) 自定义规则
# 输入域名 (如 gemini.google.com) 或 IP/CIDR (如 1.1.1.1 或 10.0.0.0/8)
```

自定义规则优先级高于预设规则，可实现精细化分流控制。

### IPv4/IPv6 出站选择

所有分流规则支持选择 IPv4 或 IPv6 出站：
...
| 仅 IPv4 | 直连流量只走 IPv4 |
| 仅 IPv6 | 直连流量只走 IPv6 |
| 优先 IPv4 | 优先 IPv4，无 IPv4 时用 IPv6 |
| 优先 IPv6 | 优先 IPv6，无 IPv6 时用 IPv4 |
...
https://你的域名:8443/sub/随机UUID/v2ray   # V2Ray/Loon/通用
```
...
将其他服务器的节点聚合到同一个订阅链接中：

```bash
vless
# 主菜单 → 3) 订阅服务管理 → 3) 外部节点管理
# 1) 添加分享链接
# 2) 添加订阅链接 (机场订阅)
# 3) 查看外部节点
# 4) 删除外部节点
