...
### ✨ 新功能
- 查看协议配置新增「一键展示所有分享链接」功能，快速获取所有已安装协议的分享链接
- Netflix 分流新增「仅使用 IPv6」选项，避免同户检测和封号风险

### 🐛 修复
- 修复 NaïveProxy 安装失败的问题
- 修复主菜单分流显示问题
...
Netflix 分流支持「仅使用 IPv6」选项，避免同户检测和封号风险：

```bash
vless
# 分流管理 → 配置分流规则 → 添加分流规则 → 2) Netflix → 选择出口 → 是否仅使用 IPv6? [y]
```
